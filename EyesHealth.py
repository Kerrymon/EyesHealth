...
if __name__ == "__main__":
    ...
    def info_msg():
        ...
        lbl_info = Label(info, text="1. Кнопка Start Eyegym включает запись с вебкамеры, записывая и сохраняя на компьютер видео.\n "
                                    ...
                                    "3. Кнопка Quit завершает деятельность программы. \n"
                                    "4. Кнопка Start Button запускает/останавливает таймер, при каждом нажатии отправляется сообщение в закрытый телеграм канал.",  font=("Helvetica", 16, "bold"))
        lbl_info.pack()


    def quit_msg():
        qw = Tk()
        qw.title("Quit Window")
        root.geometry("500x400")
        lbl_quit = Label(qw, text="Are you sure you want to quit?")
        lbl_quit.pack()
        qw.resizable(width=False, height=False)
        yes_btn = Button(qw, text="Yes", command=quit, width=10)
        yes_btn.pack(padx=10, pady=10, side=LEFT)
        no_btn = Button(qw, text="No", command=qw.destroy, width=10)
    ...
    quit_button = tk.Button(root, text="Quit", command=quit_msg, height=3, width=20).pack()
    info_button = tk.Button(root, text="Info", command=info_msg, height=3, width=20).pack()

    patch_v = tk.Label(root, text="patch 0.15", font=("Helvetica", 12, "bold"))
